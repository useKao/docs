...
To invite it, go to **Bots**, choose **Invite Kao Bot**, select a server, approve the Discord invite, and return to Kao for verification.
...
3. Open **Bot** and, under **Privileged Gateway Intents**, enable **Server Members Intent**.
4. Reset and copy the bot token.
5. Paste the token into Kao when adding your bot.
6. Invite the bot to each server you want Kao to manage.
...
For BYOB setup, enable **Server Members Intent** when Kao asks for it. Without that intent, Kao may not be able to load the bot's servers after token validation.

## Permission and install health

The dashboard can show a warning when:

- The bot is missing a required Discord permission.
- The bot is offline or still starting, so Kao cannot check it yet.
- The bot may no longer be in the selected server.

Open the warning to review the affected server. Reinvite or update the bot permissions when Kao offers that action, wait briefly if the bot is starting, and run the check again after Discord finishes applying the change.
...
| Permission check is unavailable | Check that the bot is online, wait for startup to finish, then check again |
| Bot may no longer be in the server | Invite the bot again and return to Kao to verify the server |
